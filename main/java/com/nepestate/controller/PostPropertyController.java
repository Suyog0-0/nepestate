...
import com.nepestate.service.RolePropertyService;
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            if (validationMessage != null) {
                ...
                if (propertyId > 0) {
                    // Now create the role_property association with dynamic role ID retrieval
                    RolePropertyService rolePropertyService = new RolePropertyService();
                    CustomerService customerService = new CustomerService(); // New service to handle role operations
                    int roleId = -1;
                    
                    if (admin != null) {
                        // Get admin role ID dynamically
                        roleId = customerService.getRoleIdByAdminId(admin.getAdminID());
                        ...
                        if (roleId > 0) {
                            ...
                            handleError(request, response, "Admin role not found. Please contact system administrator.");
                        }
                        
                    } else if (customer != null) {
                        // Get customer role ID dynamically
                        roleId = customerService.getRoleIdByCustomerId(customer.getCustomerID());
                        ...
                        if (roleId > 0) {
                            ...
                            handleError(request, response, "Customer role not found. Please contact system administrator.");
                        }
